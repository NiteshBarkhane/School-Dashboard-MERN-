...
import { deleteTeacher, getAllTeacher } from "../../api/apis";
...
const TeacherTable = () => {
  ...
  async function getTeachersData() {
    setLoading(true);
    const request = requestHandler(getAllTeacher);
    request()
      .then((res) => {
        setUsers(res.data);
      })
  ...
  function handleViewModal(userId) {
    navigate(`/${loggedUser.role}/teacher/view/${userId}`);
  }

  // Edit user
  function handleEdit(userId) {
    navigate(`/${loggedUser.role}/teacher/edit/${userId}`);
  }

  // Delete user
  async function handleDelete(userId) {
    setLoading(true);
    const request = requestHandler(deleteTeacher);
    request(userId)
      .then((res) => {
      ...
      <div className="text-sm">
        ...
        <table className="w-full border-b border-collapse border-b-slate-300 border-solid">
          ...
          <tbody className="font-semibold">
            ...
            {listingUsers.map((teacher, index) => (
              <tr
                key={teacher._id}
              ...
              >
                ...
                <td className="p-4">
                  <div className="w-14 rounded-full border border-solid border-gray-300 overflow-hidden">
                    <img
                ...
                <td className="p-4 text-center whitespace-normal overflow-visible text-clip">
                  <div className="relative w-fit mx-auto">
                    ...
                    <button
                      ...
                      onClick={() => toggleOption(teacher._id)}
                    >
                      <Icons.SlOptions />
                    </button>
                    {/* User option box */}
                    {activeOption == teacher._id && (
                      <div
                        ref={activeOptionRef}
                      ...
                      >
                        <ul>
                          <button
                            onClick={() => handleViewModal(teacher._id)}
